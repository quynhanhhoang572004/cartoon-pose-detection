...
class PoseHead(nn.Module):
    ...
    def __init__(self,
        ...
        # ============================================================== #
        #  OUR CONTRIBUTION (1/4): variance-aware support aggregation      #
        #  Motivation: stock PoseAnything fuses the K support shots with a #
        #  plain unweighted mean (see forward()). Under cartoon squash &   #
        #  stretch the same keypoint varies wildly across shots, so the    #
        #  mean washes out and is dragged by occluded/garbage shots. We    #
        #  make the shot-aggregation selectable and add robust variants.   #
        #    'mean'     : original PoseAnything behaviour (baseline).       #
        #    'weighted' : param-free, weight each shot by its support-      #
        #                 heatmap energy (a visibility/confidence proxy).   #
        #    'attn'     : learnable attention over shots, invisible-keypoint#
        #                 shots masked out before the softmax (our method). #
        # ============================================================== #
        ...
        # --- OUR CONTRIBUTION (2/4): learnable per-shot scorer (agg='attn') ---
        # Scores each shot's per-keypoint embedding; a softmax over shots turns
        # these scores into the aggregation weights. ~in_channels params only,
        # so it stays safe in the 20-image few-shot regime.
        if self.keypoint_agg == 'attn':
            self.shot_attn = Linear(self.in_channels, 1)
    ...
    #  the paper's core novelty: the fused token is what the Graph           #
    #  Transformer uses as its keypoint query, so a sharper, outlier-robust  #
    #  aggregation directly improves localization in the many-shot /         #
    #  high-variance cartoon setting.                                        #
    # ==================================================================== #
    def _aggregate_shots(self, query_embed_list, shot_energy_list):
        """Aggregate per-keypoint support embeddings over the K shots.

        Args:
            query_embed_list: list of K tensors, each [bs, num_kpt, C].
            shot_energy_list: list of K tensors, each [bs, num_kpt]; per-shot
                support-heatmap energy used as a visibility/confidence proxy.
        Returns:
            Tensor [bs, num_kpt, C] — the aggregated support keypoint tokens.
        """
        emb = torch.stack(query_embed_list, dim=0)          # [K, bs, num_kpt, C]

        # K==1: nothing to aggregate; every method collapses to the single shot.
        if emb.shape[0] == 1 or self.keypoint_agg == 'mean':
            return emb.mean(0)

        energy = torch.stack(shot_energy_list, dim=0)       # [K, bs, num_kpt]

        if self.keypoint_agg == 'weighted':
            # param-free: weight each shot by its (normalized) heatmap energy.
            w = energy / (energy.sum(0, keepdim=True) + 1e-8)
            return (emb * w.unsqueeze(-1)).sum(0)

        # 'attn': learnable score per shot, with invisible shots masked out.
        score = self.shot_attn(emb).squeeze(-1)             # [K, bs, num_kpt]
        score = score.masked_fill(energy < 1e-6, float('-inf'))
        attn = torch.softmax(score, dim=0)
        # guard: a keypoint invisible in *every* shot yields all -inf -> nan.
        attn = torch.nan_to_num(attn, nan=0.0)
        return (emb * attn.unsqueeze(-1)).sum(0)
    ...
    def forward(self, x, feature_s, target_s, mask_s, skeleton):
        ...
        for i, (feature, target) in enumerate(zip(feature_s, target_s)):
            ...
            resized_feature = resize(
                ...
                mode='bilinear',
                align_corners=False)
            # per-keypoint support-heatmap energy BEFORE normalization; a proxy
            # for whether this keypoint is visible/annotated in this shot.
            shot_energy_list.append(target.flatten(2).sum(-1))  # [bs, num_kpt]
        ...
        # --- OUR CONTRIBUTION (4/4): aggregate shots via _aggregate_shots ---
        # Original PoseAnything here was simply:
