...
def main():
    ...
    def build_support(ann):
        ...
        vis = kp[:, 2] > 0
        kp_xy = torch.tensor(kp[:, :2]).float()
        kp_pad = kpts_to_pad(kp_xy, H, W)
        ...
        for q in anns[1:1 + a.per_cat]:
            ...
            data = {
                ...
                'img_metas': [{'sample_skeleton': [SKELETON], 'query_skeleton': SKELETON,
                               'sample_joints_3d': [s_kp3d], 'query_joints_3d': s_kp3d,
                               'sample_center': [s_kp3d[:, :2].mean(0)],
                               'query_center': s_kp3d[:, :2].mean(0),
                               'sample_scale': [s_kp3d[:, :2].max(0)[0] - s_kp3d[:, :2].min(0)[0]],
                               'query_scale': s_kp3d[:, :2].max(0)[0] - s_kp3d[:, :2].min(0)[0],
                               'sample_rotation': [0], 'query_rotation': 0,
                               'sample_bbox_score': [1], 'query_bbox_score': 1,
                               'query_image_file': '', 'sample_image_file': ['']}]}
            with torch.no_grad():
                out = model(**data)
            vis_s = s_img[0].detach().cpu().numpy().transpose(1, 2, 0)
            vis_q = q_t[0].detach().cpu().numpy().transpose(1, 2, 0)
            plot_results(vis_s, vis_q, s_kp3d, s_w[0], None, s_w[0], SKELETON,
                         None, torch.tensor(out['points']).squeeze(0), out_dir=out_dir)
            print(f'[{id2cat[cat]}] support={qi["file_name"]} done')
