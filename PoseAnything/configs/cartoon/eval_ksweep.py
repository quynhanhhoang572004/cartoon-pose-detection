# SMOKE TEST -- OLD architecture (stock PoseAnything: unweighted torch.mean).
# Runs on the MERGED dataset (Tom & Jerry + 4 cartoon characters).
# This config lives in the ORIGINAL PoseAnything/, so keypoint_head has NO
# keypoint_agg field -> it uses the original mean aggregation. Pair it with
# PoseAnything_ours/configs/cartoon/smoke_merged.py (new arch) for the A/B.
#
#   cd PoseAnything && python setup.py develop
#   python train.py --config configs/cartoon/smoke_merged.py --work-dir work_dirs/smoke_old
#   python test.py  configs/cartoon/smoke_merged.py work_dirs/smoke_old/latest.pth

import os
NUM_SHOTS = int(os.environ.get('KSHOT', 5))   # set via: KSHOT=10 python test.py ...
...
model = dict(
    ...
    keypoint_head=dict(
        type='PoseHead',
        in_channels=768,
        # NO keypoint_agg here -> original mean aggregation (the baseline)
        transformer=dict(
...
data = dict(
    ...
    val=dict(
        ...
        data_cfg=data_cfg, valid_class_ids=VALID_CLASS_IDS,
        max_kpt_num=channel_cfg['max_kpt_num'], num_shots=NUM_SHOTS,
        num_queries=1, num_episodes=50, pipeline=valid_pipeline),
    test=dict(
